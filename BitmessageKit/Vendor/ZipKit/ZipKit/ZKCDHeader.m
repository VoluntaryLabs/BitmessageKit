...
- (id) init {
	if (self = [super init]) {
		self.magicNumber = (UInt32)ZKCDHeaderMagicNumber;
...
- (void) observeValueForKeyPath:(NSString *)keyPath ofObject:(id)object change:(NSDictionary *)change context:(void *)context {
	if ([keyPath isEqualToString:@"compressedSize"]
		...
		self.filenameLength = (UInt32)[self.filename zk_precomposedUTF8Length];
	else if ([keyPath isEqualToString:@"comment"] && self.commentLength < 1)
		self.commentLength = (UInt32)[self.comment zk_precomposedUTF8Length];
}
