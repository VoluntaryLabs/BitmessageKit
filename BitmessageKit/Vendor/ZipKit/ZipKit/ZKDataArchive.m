...
- (NSInteger) deflateFile:(NSString *)path relativeToPath:(NSString *)basePath usingResourceFork:(BOOL)rfFlag {
	...
	cdHeaderData.externalFileAttributes = (UInt32)[self.fileManager zk_externalFileAttributesAtPath:path];
...
- (NSInteger) deflateData:(NSData *)data withFilename:(NSString *)filename andAttributes:(NSDictionary *)fileAttributes {
	...
	lfHeaderData.filenameLength = (UInt32)[lfHeaderData.filename zk_precomposedUTF8Length];
	lfHeaderData.crc = (UInt32)[data zk_crc32];
	...
	if (fileAttributes) {
		...
		cdHeaderData.externalFileAttributes = (UInt32)[self.fileManager zk_externalFileAttributesFor:fileAttributes];
	}
